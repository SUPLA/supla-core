...
const string supla_abstract_restart_device_command::get_command_name(void) {
  return "RESTART-DEVICE:";
}

void supla_abstract_restart_device_command::on_command_match(
    const char *params) {
  int user_id = 0;
  int device_id = 0;
  ...
  if (params) {
    ...
    if (user_id && device_id &&
        restart_device(user_id, device_id, &queued_at, &waiting_for_result)) {
