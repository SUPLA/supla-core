...
const std::string supla_abstract_recalibrate_command::get_command_name(void) {
  return "RECALIBRATE:";
}

void supla_abstract_recalibrate_command::on_command_match(const char *params) {
  process_parameters(
      params, [this](int user_id, int device_id, int channel_id) -> bool {
        if (recalibrate(user_id, device_id, channel_id)) {
