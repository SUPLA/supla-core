...
const string supla_abstract_enter_cfg_mode_command::get_command_name(void) {
  return "ENTER-CONFIGURATION-MODE:";
}

void supla_abstract_enter_cfg_mode_command::on_command_match(
    const char *params) {
  int user_id = 0;
  int device_id = 0;
  ...
  if (params) {
    ...
    if (user_id && device_id &&
        enter_cfg_mode(user_id, device_id, &queued_at, &waiting_for_result)) {
