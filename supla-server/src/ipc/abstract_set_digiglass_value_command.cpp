...
const string supla_abstract_set_digiglass_value_command::get_command_name(
    void) {
...
void supla_abstract_set_digiglass_value_command::on_command_match(
  ...
  if (params) {
    ...
    if (user_id && device_id && channel_id) {
      if (set_digiglass_value(user_id, device_id, channel_id,
                              active_bits & 0xFFFF, mask & 0xFFFF)) {
