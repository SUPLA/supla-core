...
#include "user/user.h"
#include "vbt/abstract_vbt_condition.h"
...
class supla_value_based_trigger {
 ...
 public:
  explicit supla_value_based_trigger(int channel_id, int func,
                                     _subjectType_e subject_type,
                                     int subject_id);
  virtual ~supla_value_based_trigger(void);

  bool are_conditions_met(int channel_id, supla_channel_value *old_value,
                          supla_channel_value *new_value);

  void fire(supla_abstract_action_executor *action_executor,
