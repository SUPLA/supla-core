...
TEST_F(ChannelFragmentTest, defaultContructor) {
  ...
  EXPECT_EQ(f.get_channel_number(), 0);
...
TEST_F(ChannelFragmentTest, constructorWithArgs) {
  supla_channel_fragment f1(1, 2, 10, 3, 4, 5, false);
  EXPECT_EQ(f1.get_device_id(), 1);
  EXPECT_EQ(f1.get_channel_id(), 2);
  EXPECT_EQ(f1.get_channel_number(), 10);
...
TEST_F(ChannelFragmentTest, channelAssignment) {
  ...
  supla_device_channel channel(&device, 686, 5, SUPLA_CHANNELTYPE_ACTIONTRIGGER,
  ...
  EXPECT_EQ(f.get_channel_number(), 5);
