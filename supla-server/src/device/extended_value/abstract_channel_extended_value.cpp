...
#include "abstract_channel_extended_value.h"
...
supla_abstract_channel_extended_value::supla_abstract_channel_extended_value(
    void)
    : supla_vbt_value() {
  this->real_size = 0;
  this->value = nullptr;
...
char supla_abstract_channel_extended_value::get_type() {
  return get_value_ptr() ? get_value_ptr()->type : 0;
}

TSuplaChannelExtendedValue *supla_abstract_channel_extended_value::_realloc(
...
supla_abstract_channel_extended_value::get_value_ptr(void) {
  return value;
}

bool supla_abstract_channel_extended_value::get_raw_value(
...
void supla_abstract_channel_extended_value::set_raw_value(
    const TSuplaChannelExtendedValue *value) {
  if (value) {
    _realloc(value->size);
    if (this->value) {
...
size_t supla_abstract_channel_extended_value::get_real_size(void) {
  return real_size;
}

size_t supla_abstract_channel_extended_value::get_value_size(void) {
  return value ? value->size : 0;
}

bool supla_abstract_channel_extended_value::is_differ(
...
void supla_abstract_channel_extended_value::get_value(char *buffer) {
  if (get_value_size()) {
