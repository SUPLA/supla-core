...
supla_scene_state::supla_scene_state(
  ...
  unsigned long long ending_at_usec = started_at.tv_sec * 1000000ULL +
                                      started_at.tv_usec +
                                      estimated_execution_time_ms * 1000ULL;

  this->ending_at.tv_sec = ending_at_usec / 1000000ULL;
  this->ending_at.tv_usec = ending_at_usec % 1000000ULL;
  this->caller = caller;
}
...
unsigned int supla_scene_state::get_milliseconds_from_start(void) const {
  if (started_at.tv_sec || started_at.tv_usec) {
    ...
    unsigned long long started_at_usec =
        started_at.tv_sec * 1000000ULL + started_at.tv_usec;
...
unsigned int supla_scene_state::get_milliseconds_left(void) const {
  if (ending_at.tv_sec || ending_at.tv_usec) {
    ...
    unsigned long long ending_at_usec =
        ending_at.tv_sec * 1000000ULL + ending_at.tv_usec;
