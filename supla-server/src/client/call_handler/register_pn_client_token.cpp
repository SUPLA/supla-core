...
void supla_ch_register_pn_client_token::handle_call(
  ...
  if (rd->data.cs_register_pn_client_token &&
      rd->data.cs_register_pn_client_token->TokenSize <=
    ...
    dao.update_client_push_notification_token(client->get_id(),
                                              (char*)reg->Token, reg->Platform,
                                              reg->AppId, reg->DevelopmentEnv);
  }
}
