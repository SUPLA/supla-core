...
supla_ch_timer_arm::supla_ch_timer_arm(void)
...
void supla_ch_timer_arm::handle_call(shared_ptr<supla_client> client,
  ...
  if (rd->data.cs_timer_arm_request != NULL) {
    ...
    client->get_channels()->device_access(
        request->ChannelID, [client, request](supla_device* device) -> void {
          device->get_channels()->timer_arm(
              supla_caller(ctClient, client->get_id()), request->ChannelID, 0,
