...
void database::get_client_channels(int ClientID, int *DeviceID, supla_client_channels *channels) {
	...
	MYSQL_STMT *stmt;
	const char sql1[] = "SELECT `id`, `func`, `iodevice_id`, `location_id`, `caption` FROM `supla_v_client_channel` WHERE `client_id` = ? ORDER BY `iodevice_id`, `channel_number`";
	const char sql2[] = "SELECT `id`, `func`, `iodevice_id`, `location_id`, `caption` FROM `supla_v_client_channel` WHERE `client_id` = ? AND `iodevice_id` = ? ORDER BY `channel_number`";
	...
	if ( stmt_execute((void**)&stmt, DeviceID ? sql2 : sql1, pbind, DeviceID ? 2 : 1, true) ) {
		...
		int id, func, iodevice_id, location_id;
		unsigned long size;
		char caption[401];
		...
		rbind[3].buffer_type= MYSQL_TYPE_LONG;
		rbind[3].buffer= (char *)&location_id;

		rbind[4].buffer_type= MYSQL_TYPE_STRING;
		rbind[4].buffer= caption;
		rbind[4].is_null= &is_null;
		rbind[4].buffer_length = 401;
		rbind[4].length = &size;
		...
		if ( mysql_stmt_bind_result(stmt, rbind) ) {
			...
			if ( mysql_stmt_num_rows(stmt) > 0 ) {
				...
				while (!mysql_stmt_fetch(stmt)) {
					...
					channels->update_channel(id, iodevice_id, location_id, func, is_null ? NULL : caption);
				}
