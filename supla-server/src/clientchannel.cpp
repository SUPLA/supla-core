...
supla_client_channel::supla_client_channel(int Id, int DeviceId, int LocationID, int Func, const char *Caption) {
...
void supla_client_channel::mark_for_remote_update(char mark) {
	...
	case SUPLA_CHANNELFNC_NOLIQUIDSENSOR:
		...
		if ( mark == CC_REMOTEUPDATE_CHANNELVALUE
...
void supla_client_channels::update_channel(int Id, int DeviceId, int LocationID, int Func, const char *Caption) {
	...
	if ( ( channel = find_channel(Id) ) == NULL ) {
		channel = new supla_client_channel(Id, DeviceId, LocationID, Func, Caption);
