...
// #define ONLY_LOG_REQUESTS
     ...
     Do not use this macro except for the debug profile
...
typedef struct {
     ...
     static const _alexa_code_t alexa_codes[]{
         ...
         {(char *)"SKILL_DISABLED_EXCEPTION",
          POST_RESULT_SKILL_DISABLED_EXCEPTION},
     ...
     void supla_alexa_client::httpsInit(void) {
       httpsFree();
       lck_lock(lck);
     ...
     void supla_alexa_client::httpsFree(void) {
       lck_lock(lck);
       if (https) {
         delete https;
         https = NULL;
     ...
     void supla_alexa_client::terminate(void) {
       lck_lock(lck);
       if (https) {
         https->terminate();
       }
     ...
     supla_alexa_client::~supla_alexa_client() { lck_free(lck); }
         ...
         if (https->getResultCode() == 200 && https->getBody()) {
           ...
           if (root) {
             ...
             if (access_token && cJSON_IsString(access_token) &&
                 refresh_token && cJSON_IsString(refresh_token)) {
               int ex_in =
                   cJSON_IsNumber(expires_in) ? expires_in->valueint : 0;
     ...
     void *supla_alexa_client::getBrightnessControllerProperties(
         short brightness) {
       char now[64];
       st_get_zulu_time(now);
     ...
     void *supla_alexa_client::getPercentageControllerProperties(
         short percentage) {
       char now[64];
       st_get_zulu_time(now);
     ...
     void *supla_alexa_client::getResponseHeader(
         const char correlationToken[]) {
     ...
     bool supla_alexa_client::sendChangeReport(int causeType, int channelId,
       ...
       if (online) {
         context_props =
             addProps(context_props, getEndpointHealthProperties(true));
       }

       change_props =
           addProps(change_props, getEndpointHealthProperties(online));
     ...
     bool supla_alexa_client::sendPowerChangeReport(int causeType,
                                                    int channelId, bool hi,
                                                    bool online) {
       void *context_props = NULL;
       if (online) {
         context_props =
             addProps(context_props, getPowerControllerProperties(hi));
       }

       void *change_props = NULL;
       if (online) {
         change_props =
             addProps(change_props, getPowerControllerProperties(hi));
       }

       return POST_RESULT_SUCCESS == sendChangeReport(causeType, channelId,
                                                      online, context_props,
                                                      change_props);
     }

     bool supla_alexa_client::sendContactChangeReport(int causeType,
                                                      int channelId, bool hi,
                                                      bool online) {
       void *context_props = NULL;
       if (online) {
         context_props =
             addProps(context_props, getContactSensorProperties(hi));
       }
     ...
     bool supla_alexa_client::sendBrightnessChangeReport(int causeType,
       ...
       if (online) {
         context_props = addProps(
             context_props, getBrightnessControllerProperties(brightness));
         context_props = addProps(context_props,
                                  getPowerControllerProperties(brightness > 0));
       }

       void *change_props = NULL;
       if (online) {
         change_props = addProps(change_props,
                                 getBrightnessControllerProperties(brightness));
         change_props = addProps(change_props,
                                 getPowerControllerProperties(brightness > 0));
       }
       ...
       return POST_RESULT_SUCCESS == sendChangeReport(causeType, channelId,
                                                      online, context_props,
                                                      change_props, subChannel);
     }

     bool supla_alexa_client::sendColorChangeReport(int causeType,
                                                    int channelId, int color,
                                                    short colorBrightness,
                                                    bool online,
                                                    short subChannel) {
       if (!online) {
         colorBrightness = 0;
       ...
       void *context_props = NULL;
       if (online) {
         context_props = addProps(context_props, getColorControllerProperties(
                                                     color, colorBrightness));
         context_props = addProps(
             context_props, getBrightnessControllerProperties(colorBrightness));
         context_props = addProps(
             context_props, getPowerControllerProperties(colorBrightness > 0));
       }

       void *change_props = NULL;
       if (online) {
         change_props = addProps(change_props, getColorControllerProperties(
                                                   color, colorBrightness));
         change_props = addProps(
             change_props, getBrightnessControllerProperties(colorBrightness));
         change_props = addProps(
             change_props, getPowerControllerProperties(colorBrightness > 0));
       }
     ...
     bool supla_alexa_client::sendPercentageChangeReport(int causeType,
       ...
       if (online) {
         context_props = addProps(
             context_props, getPercentageControllerProperties(percentage));
       }

       void *change_props = NULL;
       if (online) {
         change_props = addProps(change_props,
                                 getPercentageControllerProperties(percentage));
       }
     ...
     void *supla_alexa_client::getEndpointUnrechableErrorResponse(
       ...
       if (root) {
         ...
         if (event) {
           cJSON *header =
               static_cast<cJSON *>(getErrorHeader(correlationToken));
           if (header) {
             cJSON_AddItemToObject(event, "header", header);
           }
     ...
     void *supla_alexa_client::getResponse(const char correlationToken[],
       ...
       if (root) {
         ...
         if (event) {
           cJSON *header =
               static_cast<cJSON *>(getResponseHeader(correlationToken));
           if (header) {
             cJSON_AddItemToObject(event, "header", header);
           }
     ...
     bool supla_alexa_client::brightnessControllerSendResponse(
       ...
       return POST_RESULT_SUCCESS == sendResponse(correlationToken, channelId,
                                                  online, props, subChannel);
     }
     ...
     bool supla_alexa_client::colorControllerSendResponse(
       ...
       if (online) {
         props = addProps(props,
                          getColorControllerProperties(color, colorBrightness));
         props = addProps(props,
                          getBrightnessControllerProperties(colorBrightness));
         props =
             addProps(props, getPowerControllerProperties(colorBrightness > 0));
       }
