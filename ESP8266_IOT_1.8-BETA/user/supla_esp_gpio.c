...
char supla_esp_gpio_relay_hi(int port, char hi) {
    ...
    if ( hi == 255 ) {
    	hi = supla_esp_gpio_is_hi(port) == 1 ? 0 : 1;
    }
...
LOCAL void supla_esp_gpio_relay_switch(supla_input_cfg_t *input_cfg) {
	...
	if (  input_cfg->relay_gpio_id != 255 ) {
		...
		if ( input_cfg->channel != 255 )
			supla_esp_channel_value_changed(input_cfg->channel, supla_esp_gpio_is_hi(input_cfg->relay_gpio_id));
...
supla_esp_gpio_intr_handler(void *params) {
	...
	for(a=0;a<INPUT_MAX_COUNT;a++) {
		...
		supla_esp_key_intr_zam_handler(gpio_status, ZAM_INPUT1, supla_esp_cfg.Button1Type);
	#endif

	#if defined(ZAM_INPUT2)
		supla_esp_key_intr_zam_handler(gpio_status, ZAM_INPUT2, supla_esp_cfg.Button2Type);
	#endif
	...
	for (a=0; a<RELAY_MAX_COUNT; a++)
		if ( supla_relay_cfg[a].gpio_id != 255 ) {
			...
			if ( supla_relay_cfg[a].flags & RELAY_FLAG_RESTORE_FORCE ) {
				...
				supla_esp_gpio_relay_hi(supla_relay_cfg[a].gpio_id, RELAY_INIT_VALUE);
