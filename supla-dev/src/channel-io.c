...
char channelio_allowed_type(int type) {
	...
	switch(type) {
	case SUPLA_CHANNELTYPE_RELAYHDF4:
...
char channelio_start_gpio_thread(TDeviceChannel *channel, _func_sthread_execute execute, unsigned char port, int time_ms) {
...
void channelio_channel_init(void) {
	...
	for(a=0;a<cio->channel_count;a++) {
	 ...
	 channelio_start_gpio_thread(NULL, channelio_gpio_monitor_execute, 0, 0);
#endif
...
char channelio_get_cvalue(TDeviceChannel *channel, char value[SUPLA_CHANNELVALUE_SIZE]) {
	...
	if ( channel ) {
		...
		if ( channel->type == SUPLA_CHANNELTYPE_SENSORNO
			 || channel->type == SUPLA_CHANNELTYPE_RELAYHDF4 ) {
...
#ifdef __SINGLE_THREAD
char channelio__gpio_set_timed_lo_value(TDeviceChannel *channel, unsigned char port_number, TChannelGpioPortValue *gpio_value, struct timeval *now) {
...
char channelio_gpio_set_timed_lo_value(void) {
	...
	for(a=0;a<cio->channel_count;a++) {
		if ( 1 == channelio__gpio_set_timed_lo_value(&cio->channels[a], cio->channels[a].gpio1, &cio->channels[a].gpio1_value, &now) ) {
			result = 1;
		}


		if ( 1 == channelio__gpio_set_timed_lo_value(&cio->channels[a], cio->channels[a].gpio2, &cio->channels[a].gpio2_value, &now) ) {
			result = 1;
		}
...
char channelio_set_hi_value(unsigned char number, char hi, unsigned int time_ms) {
	...
	if ( channel
		...
		if ( time_ms  == 0 || hi == 0 ) {
			channel->gpio1_value.close_time.tv_sec = 0;
			channel->gpio1_value.close_time.tv_usec = 0;
		} else if ( hi == 1 ) {

			gettimeofday(&channel->gpio1_value.close_time, NULL);
			channel->gpio1_value.close_time.tv_sec += (int)(time_ms/1000);
			channel->gpio1_value.close_time.tv_usec += (int)((time_ms%1000)*1000);

		}

		channel->gpio1_value.value = hi == 0 ? 0 : 1;
		gpio_set_value(channel->gpio1, channel->gpio1_value.value);
		lck_unlock(channel->gpio1_value.lck);
		channelio_raise_valuechanged(channel);
		result = 1;
		...
		if ( time_ms > 0 ) {
			...
			result = channelio_start_gpio_thread(channel, channelio_gpio_set_hi_execute, channel->gpio1, time_ms);
